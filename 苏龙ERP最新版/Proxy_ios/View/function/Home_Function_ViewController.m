...
-(void)usecar{
    VehicAppVC *vc =[[VehicAppVC alloc]init];
...
//            NSLog(@"%@",NEWtitle);
        ...
        }else if ([[showArray[sender.tag] objectForKey:@"MURL3"]  isEqualToString:@"epm.WCListActivity"]) {
            registerVC* vc = [[registerVC alloc] initWithUrl:[showArray[sender.tag] objectForKey:@"MURL3"] title:sender.titleLabel.text];
            vc.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:vc animated:YES];
        } else if([[showArray[sender.tag] objectForKey:@"MURL3"]  isEqualToString:@"oa.WorkTaskActivity"]){
            
                       workTaskVC*work_taskVC = [[workTaskVC alloc] initWithUrl:[showArray[sender.tag] objectForKey:@"MURL3"]  title:sender.titleLabel.text];
...
- (void)tapView//停止抖动
{
    [UIView animateWithDuration:0.2 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
         ...
         NSLog(@"完成");
         _scroolView.userInteractionEnabled = NO;
     }];
    for (id view in _scroolView.subviews) {
...
_scroolView =[[UIScrollView alloc]init];
    _scroolView.frame=CGRectMake(0, 0, LWidth, LHeight);
    //_scroolView.backgroundColor=[UIColor redColor];
    
    _scroolView.contentOffset=CGPointMake(0, 0);
    _scroolView.showsVerticalScrollIndicator=NO;
    if (showArray.count>12&showArray.count<18) {
        _scroolView.contentSize=CGSizeMake(LWidth, LHeight +LHeight/2);

    }else if(showArray.count<=12){
          _scroolView.contentSize=CGSizeMake(LWidth, LHeight);
        }else{
            _scroolView.contentSize=CGSizeMake(LWidth, LHeight*ceil(showArray.count/12));
    ...
    //[self.view addSubview:buttonView];
      [_scroolView addSubview:buttonView];
    ...
    for (NSInteger index = 0; index < showArray.count; index ++) {
        ...
        [_scroolView addSubview:btn];
        
        UIImageView* imagView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 60, 60)];
        [imagView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@%@/MobileImages/%@",HTTPIP,SLRD,[[showArray[index] objectForKey:@"GIFNAME"] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]]] placeholderImage:[UIImage imageNamed:@"index1"]];
        CGPoint point = btn.center;
        point.y = point.y-10;
        imagView.center = point;
        [_scroolView addSubview:imagView];
        ...
        [_scroolView addSubview:btnDelete];
    }
}
