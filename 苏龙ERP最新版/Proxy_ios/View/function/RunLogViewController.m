...
@interface RunLogViewController () <UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
{
...
//    NSMutableArray* dataArray;
    ...
    NSString *refrechguid;
    NSString *refrechType;;
    UIButton * reset;
...
- (void)viewDidLoad {
   ...
   // searchButton.frame = CGRectMake(valueButton.frame.size.width+valueButton.frame.origin.x+10, reset.frame.origin.y, LWidth-(valueButton.frame.size.width+valueButton.frame.origin.x)-20, 35);
...
//    dataArray = [[NSMutableArray alloc] init];
    ...
    NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:@"GETDUTYLOGDETAIL",@"Action",@"10",@"Pagesize",@"1",@"Pageindex",type,@"DATETYPE",@"DESC,DESC,ASC,ASC",@"Softtype",@"BEGINTIME,LOGID,LOGTIMEHOUR,LOGTIMEMINUTE",@"Softfield",guid,@"BOOKGUID",_textField.text,@"CONTENT", nil];
    refrechguid=guid;
    refrechType=type;
...
- (void)requestShowDataRefresh
{
    ...
    NSString* page = [NSString stringWithFormat:@"%ld",(long)startIndex];
...
//    dataArray = [[NSMutableArray alloc] init];
    NSLog(@"guid===%@",guid);
    NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:@"GETDUTYLOGDETAIL",@"Action",@"10",@"Pagesize",page,@"Pageindex",refrechType,@"DATETYPE",@"DESC,DESC,ASC,ASC",@"Softtype",@"BEGINTIME,LOGID,LOGTIMEHOUR,LOGTIMEMINUTE",@"Softfield",refrechguid,@"BOOKGUID",_textField.text,@"CONTENT", nil];
    MyRequest *manager = [MyRequest requestWithURL:[NSString stringWithFormat:@"http://%@%@/%@",HTTPIP,SLRD,PAGIN_URL] withParameter:dict];
      manager.backSuccess = ^void(NSDictionary *dictt)
    {
        [SVProgressHUD dismiss];
