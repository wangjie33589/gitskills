...
-(void)setVisibleItemList:(NSMutableArray *)visibleItemList{
    ...
    if (!self.btnBackView) {
        self.btnBackView = [[UIView alloc] initWithFrame:CGRectMake(10,(self.frame.size.height)-2,70,2)];
        self.btnBackView.backgroundColor = [UIColor colorWithRed:0.16 green:0.59 blue:1 alpha:1];
...
-(void)makeItemWithTitle:(NSString *)title{
   ...
   forControlEvents:1 << 6];
    ...
    if (!self.btnSelect) {
        [item setTitleColor:[UIColor colorWithRed:0.16 green:0.59 blue:1 alpha:1] forState:0];
        self.btnSelect = item;
    }
    self.contentSize = CGSizeMake(self.maxWidth, self.frame.size.height);
}
...
-(void)itemClick:(UIButton *)sender{
    if (self.btnSelect != sender) {
        ...
        [sender setTitleColor:[UIColor colorWithRed:0.16 green:0.59 blue:1 alpha:1] forState:0];
