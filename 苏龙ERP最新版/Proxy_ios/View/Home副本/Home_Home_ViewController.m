...
@interface Home_Home_ViewController () <LeftViewControllerDelegate,LeftViewControllerPushDelegate,UIWebViewDelegate,UITableViewDataSource,UITableViewDelegate,MyRequestDelegate>
{
    ...
    NSTimer *timer;
    NSTimer *hisTimeer;
    NSDictionary *_hisDic;
    NSDictionary *_hisDic1;
}
@end
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    hisTimeer = [NSTimer scheduledTimerWithTimeInterval:60 target:self selector:@selector(seconds_60) userInfo:nil repeats:YES];
...
//    NSString* timeXml = [NSString stringWithFormat:@"<p><a>h</a><t1>%@ 00:00:00</t1><t2>%@</t2><ts>3600</ts><sis server='SYNCBASE5'><tg n='JY1_1232AI' /><tg n='JY2_1232AI' /><tg n='JY3_2423AI' /><tg n='JY4_2423AI'/><tg n='JY5_GRLIHJ' /><tg n='JY6_6GR_QS_PNT'/></sis></p>",date,dateTime];
    NSString* timeXml = [NSString stringWithFormat:@"<p><a>h</a><t1>%@ 00:00:00</t1><t2>%@</t2><ts>3600</ts><sis server='SYNCBASE5'><tg n='JY0_QCGRZLL'/></sis></p>",date,dateTime];
//     NSString* timeXml = [NSString stringWithFormat:@"<p><a>h</a><t1>%@ 00:00:00</t1><t2>%@</t2><ts>3600</ts><sis server='SYNCBASE5'><tg n='JY0_QCGRZLL'/></sis></p>",@"2016-03-29",@"2016-03-29 00:30:00"];

    NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:@"GETSERIAL",@"Action",timeXml,@"XmlData",@"SYNCBASE5",@"EngineName", nil];
    MyRequest *manager = [MyRequest requestWithURL:[NSString stringWithFormat:@"http://%@%@/SISMonitor/ProxyMobile/GetSISData.ashx",HTTPIP,SLRD] withParameter:dict];
    NSLog(@"%@",[NSString stringWithFormat:@"http://%@/%@/SISMonitor/ProxyMobile/GetSISData.ashx",HTTPIP,SLRD]);
    ...
    manager.backSuccess = ^void(NSDictionary *dictt)
    {
        ...
        if ([[[dictt objectForKey:@"sis"] allKeys] containsObject:@"tg"])
        {
            ...
            if ([[[[dictt objectForKey:@"sis"] objectForKey:@"tg"]objectForKey:@"s"] isKindOfClass:[NSArray class]]) {
                
                
                historyArray1 = [NSMutableArray arrayWithArray:[[[dictt objectForKey:@"sis"] objectForKey:@"tg"]objectForKey:@"s"]];
                [self performSelector:@selector(addDataHis1) withObject:nil afterDelay:.5];
            }else{
                 // _hisDic =[[[dictt objectForKey:@"sis"] objectForKey:@"tg"]objectForKey:@"s"];
                
                
                NSLog(@"hisdic========%@",_hisDic);
...
//    }
        ...
        for (NSInteger row = 0; row < historyArray1.count; row ++) {
            andNumber =[[[historyArray1 objectAtIndex:row] objectForKey:@"v"] floatValue];
            if (row==historyArray1.count-1) {
                [dataArray addObject:[NSString stringWithFormat:@"%f",andNumber]];
                andNumber = 0;
            }
...
-(void)addData1{
    [self.webView stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"addData('0,0?0,0');"]];
...
//    NSString* timeXml = [NSString stringWithFormat:@"<p><a>h</a><t1>%@ 00:00:00</t1><t2>%@</t2><ts>3600</ts><sis server='SYNCBASE5'><tg n='JY1_4844AI' /><tg n='JY2_4844AI' /><tg n='JY3_3A51AI' /><tg n='JY4_3A51AI'/><tg n='JY5_5E01JA001' /><tg n='JY6_6E01JA001'/></sis></p>",date,dateTime];
    NSString* timeXml = [NSString stringWithFormat:@"<p><a>h</a><t1>%@ 00:00:00</t1><t2>%@</t2><ts>3600</ts><sis server='SYNCBASE5'><tg n='JY0_QCZGL'/></sis></p>",date,dateTime];
//     NSString* timeXml = [NSString stringWithFormat:@"<p><a>h</a><t1>%@ 00:00:00</t1><t2>%@</t2><ts>3600</ts><sis server='SYNCBASE5'><tg n='JY0_QCZGL'/></sis></p>",@"2016-03-29",@"2016-03-29 00:30:00"];
    NSLog(@"timestring======%@",timeXml);
    ...
    manager.backSuccess = ^void(NSDictionary *dictt)
    {
        ...
        if ([[[dictt objectForKey :@"sis"] allKeys] containsObject:@"tg"])
        {
            ...
            if ([[[[dictt objectForKey:@"sis"] objectForKey:@"tg"]objectForKey:@"s"] isKindOfClass:[NSArray class]]) {
                historyArray = [NSMutableArray arrayWithArray:[[[dictt objectForKey:@"sis"] objectForKey:@"tg"]objectForKey:@"s"]];
                NSLog(@"hiwsttttttarray ====%@",historyArray);
            ...
            [SVProgressHUD showErrorWithStatus:dictt[@"errorMsg"]];
...
- (void)initHistory
{
    ...
    NSMutableArray* dataArray = [[NSMutableArray alloc] init];
           for (NSInteger row = 0; row < historyArray.count; row ++) {
            andNumber = [[[historyArray objectAtIndex:row] objectForKey:@"v"] floatValue];
    ...
    NSMutableArray* dataArray1 = [[NSMutableArray alloc] init];
        ...
        for (NSInteger row = 0; row < historyArray1.count; row ++) {
            andNumber1 = [[[historyArray1 objectAtIndex:row] objectForKey:@"v"] floatValue];
            //if (row==historyArray1.count-1) {
    ...
    [SVProgressHUD dismiss];
    NSLog(@"string==================%@sdfsdfsdfdsf=======%@",dataStr,dataStr1);
...
- (void)requestShowDataList
{
    ...
    manager.backSuccess = ^void(NSDictionary *dictt)
    {
        ...
        if ([[dictt objectForKey:@"success"] integerValue] == 1) {
            if ([[NSMutableArray arrayWithArray:[[dictt objectForKey:@"sis"] objectForKey:@"tg"]] isKindOfClass:[NSArray class]]) {
            ...
            for (NSInteger index = 0; index < 4; index ++) {
                ...
                label.text = [NSString stringWithFormat:@"%.1f",[[self stringNumber:index] floatValue]];
    ...
    if (tableView.tag == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.bgImag.backgroundColor = [UIColor colorWithRed:0.15 green:0.38 blue:0.65 alpha:1];
                    cell.firstLab.text=@"机组";
            cell.Secondlab.text=@"负荷";
            cell.Thirdlab.text=@"负荷率";
            cell.fourlab.text=@"厂用电率";
            cell.fivelab.text=@"供热";
            cell.sixLab.text=@"日发电" ;
            cell.lastLabel.text=@"日供气";
...
//            cell.dosage.text = @"厂电用率";
           cell.firstLab.textColor=cell.Secondlab.textColor=cell.Thirdlab.textColor=cell.fourlab.textColor=cell.fivelab.textColor=cell.sixLab.textColor=cell.lastLabel.textColor = [UIColor colorWithRed:0.65 green:0.79 blue:0.94 alpha:1];
        }else{
            if (showDataArray.count>44) {
                
                cell.firstLab.textColor = cell.Secondlab.textColor = cell.Thirdlab.textColor = cell.fourlab.textColor = cell.fivelab.textColor = cell.sixLab.textColor =cell.lastLabel.textColor= [UIColor colorWithRed:0.89 green:0.96 blue:0.97 alpha:1];
                
                cell.firstLab.text=[NSString stringWithFormat:@"#%ld",(long)indexPath.row];
                cell.Secondlab.text=[[NSString stringWithFormat:@"%.1f",[[[ showDataArray[(indexPath.row -1)] objectForKey:@"s"] objectForKey:@"v"] floatValue]] stringByReplacingOccurrencesOfString:@"" withString:@"NaN"];
                cell.Thirdlab.text=[[NSString stringWithFormat:@"%.1f%%",[[[showDataArray[((indexPath.row -1)+18)] objectForKey:@"s"] objectForKey:@"v"] floatValue]] stringByReplacingOccurrencesOfString:@"" withString:@"NaN"];
                cell.fourlab.text=[[NSString stringWithFormat:@"%.1f%%",[[[ showDataArray[(indexPath.row -1)+12] objectForKey:@"s"] objectForKey:@"v"] floatValue]] stringByReplacingOccurrencesOfString:@"" withString:@"NaN"];
                cell.fivelab.text=[[NSString stringWithFormat:@"%.1f",[[[showDataArray[((indexPath.row -1)+6)] objectForKey:@"s"] objectForKey:@"v"] floatValue]] stringByReplacingOccurrencesOfString:@"" withString:@"NaN"];
                cell.sixLab.text=[[NSString stringWithFormat:@"%.1f",[[[ showDataArray[(indexPath.row -1)+38] objectForKey:@"s"] objectForKey:@"v"] floatValue]] stringByReplacingOccurrencesOfString:@"" withString:@"NaN"];
                cell.lastLabel.text=[[NSString stringWithFormat:@"%.0f",[[[ showDataArray[(indexPath.row -1)+44] objectForKey:@"s"] objectForKey:@"v"] floatValue]] stringByReplacingOccurrencesOfString:@"" withString:@"NaN"];
...
- (void)pushMenuViewController:(NSString *)title titleUp:(NSString *)titleStr
{
    if (title==nil||[title isEqual:[NSNull null]]||[title isEqualToString:@""]) {
        ...
        }else if ([title isEqualToString:@"epm.WCListActivity"]) {
            registerVC* vc = [registerVC new];
          vc.hidesBottomBarWhenPushed = YES;
           vc.title_Str = titleStr;
            ...
            vc.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:vc animated:YES];
        }else if ([[title substringToIndex:23] isEqualToString:@"oa.CarApplyListActivity"]) {
            
            VehicAppVC *vc =[[VehicAppVC alloc]initWithUrl:title title:titleStr];
            vc.hidesBottomBarWhenPushed=YES;
            [self.navigationController pushViewController:vc animated:YES];
