...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, LWidth-20, LHeight) style:UITableViewStylePlain];
        self.tableView.showsVerticalScrollIndicator = NO;
         [self.tableView setTableFooterView:[[UIView alloc] initWithFrame:CGRectZero]];
        [self addSubview:self.tableView];
        
        self.tableViewIndex = [[BATableViewIndex alloc] initWithFrame:(CGRect){LWidth-20,0,20,frame.size.height}];
...
- (void)setDelegate:(id<BATableViewDelegate>)delegate
{
    ...
    rect.size.height = self.tableViewIndex.indexes.count * 18;
...
- (void)reloadData
{
    ...
    rect.origin.y = (self.bounds.size.height - rect.size.height - edgeInsets.top - edgeInsets.bottom) / 2 + edgeInsets.top - IPHONE_5?30:100;
    self.tableViewIndex.frame = rect;
    self.tableViewIndex.tableViewIndexDelegate = self;
}
