...
@property (nonatomic, strong) NSString* voverStr;
...
- (id)initWithImage:(UIImage *)originalImage cropFrame:(CGRect)cropFrame limitScaleRatio:(NSInteger)limitRatio vover:(NSString *)aVover {
    self = [super init];
    if (self) {
        self.cropFrame = cropFrame;
        self.limitRatio = limitRatio;
        self.originalImage = originalImage;
        self.voverStr = aVover;
    }
    return self;
}
...
- (void)initView {
    ...
    UITapGestureRecognizer* regiontapGestureT = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(signClick)];
    [self.overlayView addGestureRecognizer:regiontapGestureT];
    
    self.ratioView = [[UIView alloc] initWithFrame:self.cropFrame];
    if ([self.voverStr isEqualToString:@"yes"]) {
        self.ratioView.frame = CGRectMake(self.cropFrame.origin.x, self.cropFrame.origin.y+100, self.cropFrame.size.width, self.cropFrame.size.width/3);
    }else{
