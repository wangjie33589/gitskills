...
- (void)viewDidLoad
{
    ...
    mainURL = @"http://app.juteh.com";
    //    mainURL = @"http://192.168.0.201:8080/app-mweb";
    [self initUI];
...
- (void)initTitleView
{
    ...
    backButton.frame = CGRectMake(10, 28, 40, 20);
    [backButton setImage:[UIImage imageNamed:@"iconfont-xiangzuo-2"] forState:UIControlStateNormal];
...
- (void)initUI
{
    ...
    [output setMetadataObjectTypes:@[AVMetadataObjectTypeQRCode,AVMetadataObjectTypeEAN13Code,AVMetadataObjectTypeUPCECode,AVMetadataObjectTypeCode39Code,AVMetadataObjectTypeCode39Mod43Code ,AVMetadataObjectTypeEAN8Code,AVMetadataObjectTypeCode93Code ,AVMetadataObjectTypeCode128Code]];
    //设置预览图层
...
- (void)setOverlayPickerView
{
    ...
    [_line setImage:[UIImage imageNamed:@"ff_QRCodeScanLine@2x"]];
    ...
    labIntroudction.text = @"请将条形码或二维码放入框内";
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (metadataObjects.count > 0)
    {
        ...
        NSLog(@"gdshjfgadjhgjhkdfhjgjfd====%@",obj);
        
        if (obj.stringValue && ![obj.stringValue isEqualToString:@""] && obj.stringValue.length > 0)
        {
            Sinaleton *single = [Sinaleton defaultSinaleton];
            single.urlAddress = obj.stringValue;
            NSLog(@"---------%@",obj.stringValue);
            NSString *mystring=obj.stringValue;
            if (![mystring isEqualToString:@""]) {
                
                
                if (isOK==NO) {
                    [self dismissViewControllerAnimated:YES completion:nil];
                    NSDictionary *dict =[NSDictionary dictionaryWithObjectsAndKeys:mystring,@"text", nil];
                    NSNotification *notification =[NSNotification notificationWithName:@"ewmtongzhi" object:nil userInfo:dict];
